...
// 		"UserService": func(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
// 			return nil
// 		},
// 	}
...
func applyEndpointTemplate(services []*descriptor.Service, registerFuncSuffix string) (string, error) {
	w := bytes.NewBuffer(nil)
	goMod := ""
	// log.Printf("IMPORTS:%s", protogen.GoImportPath(services[0].File.GoPkg.Path).String())

	services[0].File.FileDescriptorProto.GetOptions().ProtoReflect().Range(func(fd protoreflect.FieldDescriptor, v protoreflect.Value) bool {
		if fd.Name() == "go_mod_pkg" {
			goMod = v.String()
			return false
		}
		return true
	})
	if isRelativePath(services[0].File.GoPkg.Path) {
		goMod = filepath.Join(goMod, services[0].File.GoPkg.Path)
	} else {
		goMod = services[0].File.GoPkg.Path

	}
	tp := endpointParams{
		Services:           services,
		File:               services[0].File,
		RegisterFuncSuffix: registerFuncSuffix,
		GoModPkg:           services[0].File.GoPkg.Path,
	}

	if err := endpointTemplate.Execute(w, tp); err != nil {
		return "", err
	}
	return w.String(), nil
}
func applyTemplate(p param, reg *descriptor.Registry) (string, []*descriptor.Service, error) {
	w := bytes.NewBuffer(nil)
	...
	// }
	return w.String(), targetServices, nil
}
...
func NewEndpointRegisters() endpoint.EndpointRegister {
	services := map[string]endpoint.EndpointRegisterFunc{
		{{range $svc := .Services}}
		"{{$svc.GetName}}": {{.File.GoPkg.Name}}.Register{{$svc.GetName}}{{$.RegisterFuncSuffix}}FromEndpoint,
		{{end}}
	}
	return &endpointRegisters{
		serviceRegisters: services,
